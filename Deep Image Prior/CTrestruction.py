...
op_layer = odl_torch.OperatorModule(ray_trafo).to(device)
#op_adjoint_layer = odl_torch.OperatorModule(ray_trafo.adjoint).to(device)

#network setup
net = UnetModel(1,1,8,5,0).to(device)
optimizer = Adam(net.parameters(), lr=lr,weight_decay=0)
mse = torch.nn.MSELoss().to(device)
#s = sum([np.prod(list(p.size())) for p in net1.parameters()]);
#print ('Number of params: %d' % s)
#gradcheck(op_layer, torch.randn(1,1,362,362).double().requires_grad_(), eps=1e-5, atol=1e-3, check_undefined_grad=False)

#z = add_gaussian_noise(torch.zeros(1,1,362,362),std_dev=1).to(device,dtype=torch.float)

seed_num=10
seedlst=[]
seedlst.append(add_gaussian_noise(torch.zeros(1,1,362,362).to(device),std_dev=0.75))
for i in range(seed_num):
  #   seedlst.append(add_gaussian_noise(torch.zeros(1,3,512,512).to(device),std_dev=1))
    seedlst.append(add_gaussian_noise(seedlst[0],std_dev=0.25))
...
plt.imshow(gt.numpy()[0], cmap='viridis', aspect='auto')
plt.colorbar()
plt.show()

noisy_data = torch.unsqueeze(ob, 1).to(device)
data = torch.unsqueeze(gt, 1).to(device)
#noisy_data = add_gaussian_noise(data,std_dev=25)

def main(num_iter=3000,rep_psnr=True,show_evey=50,test=None,out_avg=None,exp_weight=0.99):
    for i in range(num_iter):
            k = random.randint(0, seed_num - 1)
            z = seedlst[k + 1]
       #     print(f'----------{i}--------')
